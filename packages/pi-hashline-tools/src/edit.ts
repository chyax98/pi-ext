...
const hashlineEditSchema = Type.Object(
	{
		...
		edits: Type.Optional(Type.Union([
			Type.Array(hashlineEditItemSchema, { description: "Edit operations" }),
			Type.String({ description: "JSON-stringified edits array fallback" }),
		])),
...
function normalizeEditsInput(rawEdits: unknown): { edits?: unknown[]; warning?: string; error?: string } {
	if (rawEdits === undefined) return { edits: undefined };
	if (Array.isArray(rawEdits)) return { edits: rawEdits };
	if (typeof rawEdits !== "string") return { error: "edits must be an array." };
	try {
		const parsed = JSON.parse(rawEdits) as unknown;
		if (Array.isArray(parsed)) {
			return {
				edits: parsed,
				warning: "JSON-stringified edits input was normalized to an edits array. Prefer compact JSON arrays in tool-call arguments.",
			};
		}
		return { error: "edits JSON string must parse to an array." };
	} catch (error) {
		return { error: `edits JSON string could not be parsed: ${error instanceof Error ? error.message : String(error)}` };
	}
}
...
export function registerEditTool(pi: ExtensionAPI, options: EditToolOptions = {}) {
	...
	const tool = {
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			const normalizedEdits = normalizeEditsInput(parsed.edits);
			if (normalizedEdits.error) {
				return buildEditError(absolutePath, "invalid-edit-variant", normalizedEdits.error);
			}
			const hasEditsInput = normalizedEdits.edits !== undefined;

			let edits = (normalizedEdits.edits ?? []) as any[];
			let legacyNormalizationWarning: string | undefined;
			if (normalizedEdits.warning) legacyNormalizationWarning = normalizedEdits.warning;
