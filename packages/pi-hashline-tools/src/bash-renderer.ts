...
export function registerBashRendererTool(pi: Pick<ExtensionAPI, "registerTool">, options: { cwd?: string; shellPath?: string; createBuiltInBashTool?: BuiltInFactory } = {}): any {
  ...
  const createBuiltInBashTool = options.createBuiltInBashTool ?? ((cwd: string, opts?: { shellPath?: string }) => (
    opts?.shellPath ? createBashTool(cwd, { shellPath: opts.shellPath } as any) : createBashTool(cwd)
  ));
