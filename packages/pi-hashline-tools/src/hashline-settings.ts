...
function validateSettings(raw: unknown, source: string, rawText: string): HashlineSettingsResult {
  ...
  if ("shellPath" in raw) {
    if (typeof raw.shellPath === "string" && raw.shellPath.trim().length > 0) settings.shellPath = raw.shellPath.trim();
    else warnings.push(invalid(source, "shellPath"));
  }
...
function mergeSettings(base: HashlineJsonSettings, override: HashlineJsonSettings): HashlineJsonSettings {
  ...
  if (override.shellPath !== undefined) merged.shellPath = override.shellPath;
  else if (base.shellPath !== undefined) merged.shellPath = base.shellPath;
  return merged;
}
...
export function resolveShellPath(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  return resolveHashlineJsonSettings().settings.shellPath;
}
