...
describe("repro 177 — core edit input correctness edge cases", () => {
  ...
  it("schema permits common edits[] old_text/new_text mistake so custom guidance can run", async () => {
    ...
    const editsSchema = (editTool.parameters as any).properties?.edits;
    const editsArraySchema = editsSchema?.items ? editsSchema : editsSchema?.anyOf?.find((schema: any) => schema.items);
    const bareMember = editsArraySchema?.items?.anyOf?.find(
