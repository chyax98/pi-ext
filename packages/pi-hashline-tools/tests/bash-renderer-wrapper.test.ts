...
describe("bash renderer wrapper", () => {
  ...
  it("passes configured shellPath to the built-in bash tool factory", async () => {
    const createBuiltIn = vi.fn(() => ({ execute: vi.fn(async () => ({ content: [{ type: "text", text: "" }] })) }));
    let registered: any;
    registerBashRendererTool(
      { registerTool(def: any) { registered = def; } } as any,
      { createBuiltInBashTool: createBuiltIn, cwd: "/tmp/work", shellPath: "/custom/bash" },
    );

    await registered.execute("call-1", { command: "pwd" }, undefined, undefined, { cwd: "/tmp/work" });

    expect(createBuiltIn).toHaveBeenCalledWith("/tmp/work", { shellPath: "/custom/bash" });
  });
});
