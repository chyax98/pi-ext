...
function preserveSeparatorArgs(separatorIndex: number, after: string[]): string[] {
  return separatorIndex >= 0 && after.length > 0 ? ["--", ...after] : after;
}
...
function parseNpxArgs(args: string[]): ParsedInvocation | null {
  ...
  if (sawPackageFlag) {
    ...
    const extraArgs = positionals.slice(1).concat(preserveSeparatorArgs(separatorIndex, after));
