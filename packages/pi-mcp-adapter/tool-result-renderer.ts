...
const DEFAULT_MAX_COLLAPSED_RESULT_CHARS = 1500;
...
export function formatMcpToolResultLines(
  ...
  expanded: boolean,
  maxCollapsedLines = 3,
  maxCollapsedChars = DEFAULT_MAX_COLLAPSED_RESULT_CHARS,
  ...
  const withinLineLimit = lines.length <= maxCollapsedLines;
  const collapsedLines = lines.slice(0, maxCollapsedLines);
  let remainingChars = maxCollapsedChars;
  let charTruncated = false;
  const cappedLines = collapsedLines.map((line) => {
    if (remainingChars <= 0) {
      charTruncated = true;
      return "";
    }
    ...
    charTruncated = true;
    const capped = truncateText(line, remainingChars);
    remainingChars = 0;
    return capped;
  }).filter((line) => line.length > 0);

  const truncated = !withinLineLimit || charTruncated;
  return {
    lines: truncated ? [...cappedLines, "…"] : cappedLines,
    truncated,
  };
}
