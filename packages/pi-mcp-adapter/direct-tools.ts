...
import { isAbortError, throwIfAborted, withAbort } from "./abort.ts";
...
export function createDirectToolExecutor(
  ...
  return async function execute(_toolCallId, params, signal) {
    ...
    if (!state && initPromise) {
      try {
        state = await withAbort(initPromise, signal);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        return {
    ...
    let connected = await withAbort(lazyConnect(state, spec.serverName), signal);
    let autoAuthAttempted = false;

    if (!connected && state.manager.getConnection(spec.serverName)?.status === "needs-auth") {
      autoAuthAttempted = true;
      const autoAuth = await withAbort(attemptDirectAutoAuth(state, spec.serverName), signal);
      ...
      if (autoAuth.status === "success") {
        await withAbort(state.manager.close(spec.serverName), signal);
        state.failureTracker.delete(spec.serverName);
        connected = await withAbort(lazyConnect(state, spec.serverName), signal);
      }
    }
    ...
    try {
      ...
      if (spec.resourceUri) {
        const result = await withAbort(connection.client.readResource({ uri: spec.resourceUri }), signal);
      ...
      if (isAbortError(error)) {
        ...
        return {
          ...
          details: { error: "cancelled", server: spec.serverName },
        };
      }
      const message = error instanceof Error ? error.message : String(error);
      uiSession?.sendToolCancelled(message);
