...
describe("proxy auto auth", () => {
  ...
  it("returns a cancellation result when a proxy MCP call is aborted", async () => {
    const { executeCall } = await import("../proxy-modes.ts");
    const controller = new AbortController();
    const connected = {
      status: "connected",
      client: { callTool: vi.fn(() => new Promise(() => {})) },
    };
    ...
    const pending = executeCall(state, "demo_search", { q: "hello" }, "demo", undefined, controller.signal);
    controller.abort();
