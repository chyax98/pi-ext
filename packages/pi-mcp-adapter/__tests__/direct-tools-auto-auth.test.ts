...
describe("direct tools auto auth", () => {
  ...
  it("returns a cancellation result when a direct MCP call is aborted", async () => {
    const { createDirectToolExecutor } = await import("../direct-tools.ts");
    ...
    const pending = executor("id", { q: "hello" }, controller.signal, () => {}, undefined as any);
    await vi.waitFor(() => expect(connected.client.callTool).toHaveBeenCalled());
    controller.abort();
    const result = await pending;

    expect(result.content[0].text).toBe("MCP tool call cancelled.");
