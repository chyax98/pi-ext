...
describe("cli init helper", () => {
  ...
  it("runs when invoked through a symlinked bin path", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "pi-mcp-cli-bin-"));
    const binPath = join(tempDir, "pi-mcp-adapter");
    symlinkSync(new URL("../cli.js", import.meta.url), binPath);

    const result = spawnSync(process.execPath, [binPath, "--help"], { encoding: "utf-8" });

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("pi-mcp-adapter helper");
  });
});
