...
describe("config discovery", () => {
  ...
  it("merges partial Pi per-server overrides with imported server definitions", async () => {
    const home = mkdtempSync(join(tmpdir(), "pi-mcp-config-home-"));
    const project = mkdtempSync(join(tmpdir(), "pi-mcp-config-project-"));
    process.env.HOME = home;
    process.chdir(project);
    ...
    writeJson(join(home, ".claude", "mcp.json"), {
      mcpServers: {
        doris: { command: "uvx", args: ["doris-mcp"], env: { TOKEN: "secret" } },
      },
    });
    ...
    expect(config.mcpServers.doris).toMatchObject({
      command: "uvx",
      args: ["doris-mcp"],
      env: { TOKEN: "secret" },
      directTools: true,
