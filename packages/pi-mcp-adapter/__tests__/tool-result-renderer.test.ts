...
describe("MCP tool result renderer", () => {
  ...
  it("caps collapsed single-line text results by character count", () => {
    const display = formatMcpToolResultLines(result([
    ...
    expect(display.lines.at(-1)).toBe("…");
  });

  it("renders a collapse hint for character-truncated results", () => {
    const output = renderMcpToolResult(
      result([{ type: "text", text: "x".repeat(5000) }]),
      collapsedOptions,
      plainTheme,
      { isError: false },
    ).render(80).join("\n");
