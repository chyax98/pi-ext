...
import { isAbortError, withAbort } from "./abort.ts";
...
export async function executeCall(
  ...
  if (serverName && !toolMeta) {
    const connected = await withAbort(lazyConnect(state, serverName), signal);
    if (connected) {
      toolMeta = findToolByName(state.toolMetadata.get(serverName), toolName);
    } else {
      ...
      if (needsAuthConnection?.status === "needs-auth") {
        if (!autoAuthAttempted) {
          ...
          const autoAuth = await withAbort(attemptAutoAuth(state, serverName), signal);
          if (autoAuth.status === "failed") {
            return {
              content: [{ type: "text" as const, text: autoAuth.message }],
          ...
          if (autoAuth.status === "success") {
            await withAbort(state.manager.close(serverName), signal);
            state.failureTracker.delete(serverName);
            const connectedAfterAuth = await withAbort(lazyConnect(state, serverName), signal);
  ...
  if (!connection || connection.status !== "connected") {
    ...
    try {
      ...
      connection = await withAbort(state.manager.connect(serverName, definition), signal);
      if (connection.status === "needs-auth") {
        if (!autoAuthAttempted) {
          autoAuthAttempted = true;
          const autoAuth = await withAbort(attemptAutoAuth(state, serverName), signal);
      ...
      if (isAbortError(error)) {
        return {
          content: [{ type: "text" as const, text: "MCP tool call cancelled." }],
          details: { mode: "call", error: "cancelled", server: serverName },
        };
      }
      const message = error instanceof Error ? error.message : String(error);
      return {
  ...
  try {
    ...
    if (toolMeta.resourceUri) {
      const result = await withAbort(connection.client.readResource({ uri: toolMeta.resourceUri }), signal);
    ...
    const resultPromise = withAbort(connection.client.callTool({
    ...
    if (isAbortError(error)) {
      uiSession?.sendToolCancelled("MCP tool call cancelled.");
      return {
        content: [{ type: "text" as const, text: "MCP tool call cancelled." }],
        details: { mode: "call", error: "cancelled", server: serverName },
      };
