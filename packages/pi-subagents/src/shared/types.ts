...
export interface ChildIssue {
	...
	source: "acceptance" | "runtime" | "tool" | "model" | "guard";
	code?: string;
	message: string;
	repairPrompt?: string;
}

export type ChildVerdict = "ok" | "warning" | "blocking";
...
export interface AsyncJobState {
	...
	notifiedStepKeys?: Set<string>;
	/** Poller-only UI clock bucket; forces widget redraw for live durations/ages while running. */
	lastWidgetUiClock?: number;
}
...
export const SUBAGENT_ASYNC_CHILD_COMPLETE_EVENT = "subagent:async-child-complete";
...
export const MIN_SUBAGENT_TIMEOUT_MS = 10 * 60_000;
export const DEFAULT_SUBAGENT_TIMEOUT_MS = 30 * 60_000;

export interface SubagentTimeoutInput {
	timeoutMinutes?: number;
}

export function requestedSubagentTimeoutMs(input: SubagentTimeoutInput | undefined): number | undefined {
	if (!input) return undefined;
	const minutes = input.timeoutMinutes;
...
export function resolveSubagentTimeoutMs(timeoutMs: number | undefined): number {
  if (timeoutMs === undefined) return DEFAULT_SUBAGENT_TIMEOUT_MS;
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) return DEFAULT_SUBAGENT_TIMEOUT_MS;
  return Math.max(MIN_SUBAGENT_TIMEOUT_MS, Math.floor(timeoutMs));
}

export type ChildExtensionMode = "isolated" | "inherit";

export interface ChildExtensionPolicy {
  /** Default isolated for workflow children; legacy subagent can keep inherit until call sites switch. */
  mode?: ChildExtensionMode;
  /** Extensions to load explicitly after runtime-required child extensions. */
  include?: string[];
  /** Extensions to remove from runtime/tool/agent/include explicit extension list. Cannot remove Pi auto-discovered extensions when mode='inherit'. */
  exclude?: string[];
}
...
export function resolveTempScopeId(options?: {
	...
	const explicitScope = env.PI_SUBAGENTS_TEMP_SCOPE_ID;
	if (explicitScope) return `scope-${sanitizeTempScopeSegment(explicitScope)}`;
