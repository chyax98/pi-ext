...
export function resolveNestedResumeCwd(rootRunId: string, run: NestedRunSummary): string | undefined {
	const asyncDir = resolveNestedAsyncDir(rootRunId, run);
	const status = asyncDir ? readStatus(asyncDir) : null;
	const statusCwd = typeof status?.cwd === "string" ? status.cwd : undefined;
	return statusCwd ?? (asyncDir ? path.dirname(asyncDir) : undefined);
}
...
function resolveNestedResumeTarget(match: ResolvedSubagentRunId & { kind: "nested" }, trustedSessionRoots: string[]): NestedResumeSourceTarget {
	...
	const cwd = resolveNestedResumeCwd(match.match.rootRunId, run);
	return {
		kind: "revive",
		source: "nested",
		runId: run.id,
...
async function resumeAsyncRun(input: {
	...
	const result = executeAsyncSingle(runId, {
		...
		timeoutMs: requestedSubagentTimeoutMs(input.params),
		extensionPolicy: input.params.extensionPolicy,
		availableModels,
	});
	...
	const lines = [
		...
		`Status if needed: agent_status({ id: "${revivedId}" })`,
	].filter((line): line is string => Boolean(line));
...
function runAsyncPath(data: ExecutionContextData, deps: ExecutorDeps): AgentToolResult<Details> | null {
	...
	if (hasTasks && params.tasks) {
		...
		const parallelTasks = params.tasks.map((task, index) => ({
			...
			...(requestedSubagentTimeoutMs(task) !== undefined ? { timeoutMs: requestedSubagentTimeoutMs(task) } : {}),
...
async function runParallelPath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	if (params.clarify === true && ctx.hasUI) {
		...
		if (result.runInBackground) {
			...
			const parallelTasks = tasks.map((t, i) => {
				...
				return {
					...(requestedSubagentTimeoutMs(t) !== undefined ? { timeoutMs: requestedSubagentTimeoutMs(t) } : {}),
					agent: t.agent,
					task: taskText,
					cwd: t.cwd,
	...
	try {
		...
		const aggregate = resolveAggregateOutcome({
			policy: inferAggregatePolicy({ agents: tasks.map((task) => task.agent), tasks: taskTexts }),
			results,
		});
		...
		const summary = `${aggregate.message ?? `${ok}/${results.length} usable`}${downgradeNote}`;
