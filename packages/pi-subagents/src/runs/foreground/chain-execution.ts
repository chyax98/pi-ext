		...
		if (isParallelStep(step)) {
			...
			try {
				...
				const parallelResults = await runParallelChainTasks({
					...
					worktreeSetup,
					timeoutMs: requestedSubagentTimeoutMs(step) ?? requestedSubagentTimeoutMs(params),
			...
			const r = await runSync(ctx.cwd, agents, seqStep.agent, stepTask, {
				...
				timeoutMs: requestedSubagentTimeoutMs(seqStep) ?? requestedSubagentTimeoutMs(params),
				extensionPolicy: params.extensionPolicy,
				acceptanceContext: { mode: "chain" },
				onUpdate: onUpdate
					? (p) => {
