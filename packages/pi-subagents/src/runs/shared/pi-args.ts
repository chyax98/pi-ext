...
import type { ChildExtensionPolicy, JsonSchemaObject } from "../../shared/types.ts";
...
function matchesExtension(candidate: string, pattern: string): boolean {
  if (candidate === pattern) return true;
  return path.basename(candidate) === pattern || candidate.endsWith(pattern);
}
...
function resolveExplicitExtensions(input: { runtimeExtensions: string[]; toolExtensionPaths: string[]; legacyExtensions?: string[]; policy?: ChildExtensionPolicy }): { isolated: boolean; extensions: string[] } {
  const policy = input.policy;
  const legacyExtensions = input.legacyExtensions;
  const mode = policy?.mode ?? (legacyExtensions !== undefined ? "isolated" : "inherit");
  const excluded = policy?.exclude ?? [];
  const explicit = uniqueExtensions([
    ...input.runtimeExtensions,
    ...input.toolExtensionPaths,
    ...(legacyExtensions ?? []),
    ...(policy?.include ?? []),
  ]).filter((candidate) => !excluded.some((pattern) => matchesExtension(candidate, pattern)));
  return { isolated: mode === "isolated", extensions: explicit };
}
...
export function buildPiArgs(input: BuildPiArgsInput): BuildPiArgsResult {
	...
	const extensionResolution = resolveExplicitExtensions({
    runtimeExtensions,
    toolExtensionPaths,
    legacyExtensions: input.extensions,
    policy: input.extensionPolicy,
  });
  if (extensionResolution.isolated) {
    args.push("--no-extensions");
  }
  for (const extPath of extensionResolution.extensions) {
