...
export function formatAsyncStartedMessage(headline: string): string {
	return [
		...
		"Use agent_status({ id: \"...\" }) when you need the current status/result, or to inspect a blocked/stale run. Do not poll just to wait.",
	].join("\n");
}
...
export function executeAsyncChain(
	...
	const buildSeqStep = (s: SequentialStep, sessionFile?: string, behaviorCwd?: string, progressPrecreated = false, resolvedBehavior?: ResolvedStepBehavior) => {
		...
		return {
			...
			timeoutMs: requestedSubagentTimeoutMs(s) ?? requestedSubagentTimeoutMs(params),
			extensionPolicy,
	...
	try {
		steps = chain.map((s, stepIndex) => {
			if (isParallelStep(s)) {
				...
				return {
					parallel: s.parallel.map((t, taskIndex) => {
						...
						return buildSeqStep({ ...t, timeoutMs: requestedSubagentTimeoutMs(t) ?? requestedSubagentTimeoutMs(s) }, nextSessionFile(), behaviorCwd, progressPrecreated, parallelBehaviors[taskIndex]);
					}),
			...
			if (isDynamicParallelStep(s)) {
				...
				return {
					expand: s.expand,
					parallel: buildSeqStep({ ...s.parallel, timeoutMs: requestedSubagentTimeoutMs(s.parallel) ?? requestedSubagentTimeoutMs(s) } as SequentialStep, undefined, undefined, progressPrecreated, behavior),
					collect: s.collect,
					concurrency: s.concurrency,
					failFast: s.failFast,
					phase: s.phase,
