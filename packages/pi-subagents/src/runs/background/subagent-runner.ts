...
import { inferAggregatePolicy, resolveAggregateOutcome } from "../shared/aggregate-outcome.ts";
import {
...
function runPiStreaming(
	...
	return new Promise((resolve) => {
		...
		const effectiveTimeoutMs = resolveSubagentTimeoutMs(timeoutMs);
		timeoutTimer = setTimeout(() => {
			if (settled || childExited) return;
			timedOut = true;
			forcedTerminationSignal = true;
			const timeoutMinutes = Math.round(effectiveTimeoutMs / 60_000);
			error = error ?? `Subagent timed out after ${timeoutMinutes} minute(s). Minimum timeout is 10 minutes; use timeoutMs to set a longer limit.`;
		...
		child.on("close", (exitCode, signal) => {
			...
			resolve({
				stderr,
				exitCode: interrupted || forcedDrainAfterFinalSuccess ? 0 : timedOut ? 1 : forcedTerminationSignal || signal ? (exitCode ?? 1) : exitCode,
				messages,
				usage,
				model,
...
function createShareLink(htmlPath: string): { shareUrl: string; gistUrl: string } | { error: string } {
	...
	try {
		...
		const viewerBase = process.env.PI_SUBAGENT_SESSION_VIEWER_URL?.replace(/\/+$/, "");
		const shareUrl = viewerBase ? `${viewerBase}/?${gistId}` : gistUrl;
...
async function runSingleStep(
	...
	for (let index = 0; index < candidates.length; index++) {
		...
		const { args, env, tempDir } = buildPiArgs({
			...
			extensionPolicy: step.extensionPolicy,
	...
	const blockingAcceptanceFailure = verdict.verdict === "blocking" && acceptance?.status === "rejected" && (finalResult?.exitCode ?? 1) === 0 && !finalResult?.interrupted;
	const effectiveFinalExitCode = blockingAcceptanceFailure ? 1 : finalResult?.exitCode ?? 1;
	const effectiveFinalError = blockingAcceptanceFailure
		? (finalResult?.error ? `${finalResult.error}\n${verdict.issues.find((issue) => issue.severity === "blocking")?.message ?? acceptanceFailure}` : verdict.issues.find((issue) => issue.severity === "blocking")?.message ?? acceptanceFailure)
		: finalResult?.error;
	...
	return {
		...
		acceptance,
		issues: verdict.issues.length ? verdict.issues : undefined,
...
async function runSubagent(config: SubagentRunConfig): Promise<void> {
	...
	const RUNNER_UI_HEARTBEAT_MS = 500;
	activityTimer = setInterval(() => {
		...
		for (const step of statusPayload.steps) {
			if (step.status === "running") {
				step.lastActivityAt = step.lastActivityAt ?? now;
			}
		}
		writeStatusPayload();
	}, RUNNER_UI_HEARTBEAT_MS);
	activityTimer.unref?.();
	...
	for (let stepIndex = 0; stepIndex < steps.length; stepIndex++) {
		...
		if (isParallelGroup(step)) {
			...
			try {
				...
				const parallelResults = await mapConcurrent(
					...
					async (task, taskIdx) => {
						...
						statusPayload.steps[fi].structuredOutputSchemaPath = singleResult.structuredOutputSchemaPath;
						statusPayload.steps[fi].acceptance = singleResult.acceptance;
						statusPayload.steps[fi].issues = singleResult.issues;
						statusPayload.steps[fi].verdict = singleResult.verdict;
						statusPayload.lastUpdate = taskEndTime;
						writeStatusPayload();
			...
			statusPayload.steps[flatIndex].issues = singleResult.issues;
			statusPayload.steps[flatIndex].verdict = singleResult.verdict;
			if (stepTokens) {
	...
	const aggregate = resolveAggregateOutcome({
		policy: inferAggregatePolicy({ agents: flatSteps.map((step) => step.agent), tasks: flatSteps.map((step) => step.task ?? "") }),
		results,
	});
	statusPayload.state = interrupted ? "paused" : aggregate.status === "partial" || aggregate.status === "complete" ? "complete" : "failed";
	statusPayload.activityState = undefined;
	...
	statusPayload.aggregate = aggregate;
	...
	try {
		writeAtomicJson(resultPath, {
			...
			agent: agentName,
			mode: resultMode,
			success: !interrupted && (aggregate.status === "complete" || aggregate.status === "partial"),
			state: interrupted ? "paused" : aggregate.status === "partial" || aggregate.status === "complete" ? "complete" : "failed",
			summary: interrupted ? "Paused after interrupt. Waiting for explicit next action." : aggregate.status === "partial" ? `${aggregate.message}\n\n${summary}` : summary,
			...
			aggregate,
			exitCode: interrupted || aggregate.status === "complete" || aggregate.status === "partial" ? 0 : 1,
