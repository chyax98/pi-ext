...
export function createResultWatcher(
	...
	const handleResult = async (file: string) => {
		...
		try {
			...
			const trackedJob = state.asyncJobs.get(runId);
			if (data.sessionId) {
				if (data.sessionId !== state.currentSessionId) return;
			} else {
				if (!trackedJob) return;
				if (trackedJob.sessionId && trackedJob.sessionId !== state.currentSessionId) return;
			}
			...
			const now = Date.now();
			const completionKey = buildCompletionKey({ ...data, id: runId }, `result:${file}`);
			...
			if (intercomTarget) {
				...
				if (!delivered) {
					console.error(`Subagent async grouped result intercom delivery was not acknowledged for '${resultPath}'. Keeping result file for retry.`);
					const retry = timers.setTimeout(() => state.resultFileCoalescer.schedule(file, 0), POLL_INTERVAL_MS);
					retry.unref?.();
					return;
				}
			}
			...
			for (const [index, child] of normalizedChildren.entries()) {
				if (child.status === "detached") continue;
				pi.events.emit(SUBAGENT_ASYNC_CHILD_COMPLETE_EVENT, {
					...
					taskIndex: child.index ?? index,
					totalTasks: normalizedChildren.length,
					status: child.status,
					success: child.status === "completed",
					state: child.status === "completed" ? "complete" : child.status === "failed" ? "failed" : "paused",
					summary: child.summary,
					error: data.results?.[index]?.error,
					outputFile: child.artifactPath,
					sessionFile: child.sessionPath ?? data.results?.[index]?.sessionFile,
