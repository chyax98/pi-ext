...
function resultHasWarning(result: Details["results"][number]): boolean {
...
function resultStatusLine(result: Details["results"][number], output: string): string {
	...
	if (resultHasWarning(result)) return result.issues?.find((issue) => issue.severity === "warning")?.message ?? "Completed with warning";
...
function resultGlyph(result: Details["results"][number], output: string, theme: Theme, running = result.progress?.status === "running", seed = progressRunningSeed(result.progress ?? result.progressSummary)): string {
	...
	if (resultHasWarning(result)) return theme.fg("warning", "⚠");
...
function renderMultiCompact(d: Details, theme: Theme): Component {
	...
	const failed = d.aggregate?.status === "failed" || (!d.aggregate && d.results.some((r) => r.exitCode !== 0 && r.progress?.status !== "running"))
	...
	if (!hasRunning && d.aggregate?.status === "partial" && d.aggregate.message) c.addChild(new Text(truncLine(theme.fg("warning", `  ⎿  ${d.aggregate.message}`), width), 0, 0));
	...
	for (const entry of renderEntries) {
		...
		if (rRunning && rProg && "status" in rProg) {
			...
			c.addChild(new Text(truncLine(theme.fg("dim", `    ⎿  ${activity}`), width), 0, 0));
			c.addChild(new Text(truncLine(theme.fg("accent", "    Press Ctrl+O for live detail"), width), 0, 0));
		} else if (!rPending && (r.exitCode !== 0 || resultHasWarning(r) || r.interrupted || r.detached || hasEmptyTextOutputWithoutOutputTarget(r.task, output))) {
			c.addChild(new Text(truncLine(theme.fg(r.exitCode !== 0 ? "error" : resultHasWarning(r) ? "warning" : "dim", `    ⎿  ${resultStatusLine(r, output)}`), width), 0, 0));
		}
