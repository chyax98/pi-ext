/**
 * Pi managed-agent extension.
 *
 * Public surface: agent_* tools, workflow_* tools, managed-agent slash commands,
 * prompt templates, and system-prompt guidance for delegation.
 */
...
import { Box, Container, Key, Spacer, Text, truncateToWidth, visibleWidth, wrapTextWithAnsi, type Component } from "@earendil-works/pi-tui";
...
import { createWorkflowExecutor, type WorkflowParamsLike } from "../workflow/workflow-tool.ts";
...
import { registerAgentRunTools } from "./agent-tools.ts";
import { registerWorkflowRunTools } from "./workflow-tools.ts";
import { registerManagedAgentPromptSupport } from "./managed-agent-prompt.ts";
import { requestLatestForegroundDetach } from "../runs/foreground/foreground-detach.ts";
import {
	type Details,
	type SubagentState,
	ASYNC_DIR,
...
export default function registerSubagentExtension(pi: ExtensionAPI): void {
	...
	registerManagedAgentPromptSupport(pi);
	pi.registerShortcut(Key.ctrl("b"), {
		description: "Detach active foreground child agent to background",
		handler(ctx) {
			const result = requestLatestForegroundDetach("keyboard detach");
			ctx.ui.notify(result.message, result.accepted ? "info" : "warning");
		},
	});
	...
	const executeSubagentCollapsed = (id: string, params: SubagentParamsLike, signal: AbortSignal, onUpdate: ((result: AgentToolResult<Details>) => void) | undefined, ctx: ExtensionContext) => {
		...
		const onUpdateWithRender = onUpdate && ctx.hasUI
			? (update: AgentToolResult<Details>) => {
				onUpdate(update);
				ctx.ui.requestRender?.();
			}
			: onUpdate;
		return executor.execute(id, params, signal, onUpdateWithRender, ctx);
	};
	...
	const workflowExecutor = createWorkflowExecutor(executeSubagentCollapsed);
	...
	registerWorkflowRunTools({
		pi,
		execute: (id, params, signal, onUpdate, ctx) => workflowExecutor.run(id, params as WorkflowParamsLike, signal, onUpdate, ctx),
	});
	...
	const resetSessionState = (ctx: ExtensionContext) => {
		...
		clearPendingForegroundControlNotices(state);
		resetJobs(ctx);
		restoreActiveJobs(ctx, { replace: true });
