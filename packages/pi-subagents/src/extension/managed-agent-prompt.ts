...
- \`agent_start\` accepts top-level \`agents\`, \`placement\`, \`wait\`, \`concurrency\`, \`isolation\`, and \`output\`; put timeout/model overrides inside each \`agents[].runtime\`, not as top-level \`runtime\`.
- Put \`output\` at the top level. Identical \`agents[].output\` values are tolerated and promoted, but per-agent output paths are not supported and mixed values are rejected.
