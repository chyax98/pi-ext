...
const SubagentTimeoutMinutes = Type.Integer({ minimum: 1, description: "Child agent timeout budget in minutes. Effective minimum is 10 minutes: values 1-9 are automatically raised to 10. Default is 30 minutes." });
...
const TaskItem = Type.Object({
	...
	timeoutMinutes: Type.Optional(SubagentTimeoutMinutes),
});
...
const ChildExtensionPolicySchema = Type.Object({
  mode: Type.Optional(Type.String({ enum: ["isolated", "inherit"], description: "Child Pi extension mode. isolated means --no-extensions plus explicit includes; inherit means normal Pi extension discovery plus explicit includes." })),
  include: Type.Optional(Type.Array(Type.String(), { description: "Extension paths/names to explicitly load in child Pi" })),
  exclude: Type.Optional(Type.Array(Type.String(), { description: "Extension paths/names to remove from explicit child extension list. Cannot remove Pi auto-discovered extensions in inherit mode." })),
}, { additionalProperties: false });
...
export const SubagentParams = Type.Object({
	...
	timeoutMinutes: Type.Optional(SubagentTimeoutMinutes),
	extensionPolicy: Type.Optional(ChildExtensionPolicySchema),
});
