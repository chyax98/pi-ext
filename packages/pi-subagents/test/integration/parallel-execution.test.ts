...
describe("parallel agent execution", { skip: !piAvailable ? "pi packages not available" : undefined }, () => {
	...
	it("read-only top-level parallel returns partial aggregate when one scout fails", async () => {
		mockPi.onCall({ output: "alpha" });
		mockPi.onCall({ output: "timeout", exitCode: 1 });
		mockPi.onCall({ output: "gamma" });
		const executor = makeExecutor([makeAgent("scout")]);

		const result = await executor.execute(
			"foreground-partial-scouts",
			{
				tasks: [
		...
		assert.equal(result.details?.aggregate?.status, "partial");
		assert.equal(result.details?.aggregate?.failedIndexes?.length, 1);
		assert.equal(result.details?.aggregate?.usable, 2);
		assert.equal(result.details?.aggregate?.failed, 1);
