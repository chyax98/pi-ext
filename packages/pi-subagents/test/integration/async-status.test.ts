	...
	it("formats model thinking in step summaries", () => {
		...
		try {
			createAsyncDir(root, "run-model", {
				...
				lastUpdate: 200,
				steps: [
					{ agent: "reviewer", status: "running", model: "provider/slow-model:high" },
					{ agent: "scout", status: "running", model: "provider/fast-model", thinking: "low" },
					{ agent: "local", status: "running", model: "local/local-coder:7b" },
					{ agent: "fallback", status: "running", model: "provider/balanced-model:low", thinking: "high" },
				],
			});

			const text = formatAsyncRunList(listAsyncRuns(root, { states: ["running"] }));
			assert.match(text, /1\. reviewer \| running \| slow-model · thinking high/);
			assert.match(text, /2\. scout \| running \| fast-model · thinking low/);
			assert.match(text, /3\. local \| running \| local-coder:7b(?! · thinking)/);
			assert.match(text, /4\. fallback \| running \| balanced-model · thinking low/);
