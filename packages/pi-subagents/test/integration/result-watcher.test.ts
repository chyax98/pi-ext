...
describe("result watcher", () => {
	...
	it("does not deliver cwd-only results to an unrelated current session", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-result-watcher-wrong-session-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
			const pi = {
				events: {
			...
			state.currentSessionId = "session-current";
			state.baseCwd = "/repo";
			const resultPath = path.join(resultsDir, "cwd-only-run.json");
			fs.writeFileSync(resultPath, JSON.stringify({
				id: "cwd-only-run",
				cwd: "/repo",
				success: true,
	...
	it("delivers sessionless results only when the run is already tracked in the current session", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-result-watcher-known-job-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
			const pi = {
				events: {
			...
			state.currentSessionId = "session-current";
			state.baseCwd = "/repo";
			state.asyncJobs.set("known-run", { asyncId: "known-run", asyncDir: path.join(resultsDir, "known-run"), status: "running", updatedAt: Date.now(), sessionId: "session-current" });
			const resultPath = path.join(resultsDir, "known-run.json");
			fs.writeFileSync(resultPath, JSON.stringify({
				id: "known-run",
				cwd: "/repo",
				success: true,
	...
	it("emits async completion plus one grouped intercom result event when an intercom target is present", async () => {
		...
		try {
			...
			assert.match(message, /Revive child: agent_send\(\{ id: "async-1", index: 0, input: "\.\.\." \}\)/);
	...
	it("keeps unacknowledged grouped async intercom results for retry before completing", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-result-watcher-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
				...
				events: {
					...
					emit(event: string, data: unknown) {
						...
						if (event === "subagent:result-intercom") {
							intercomAttempts++;
							const requestId = data && typeof data === "object" ? (data as { requestId?: unknown }).requestId : undefined;
							if (intercomAttempts === 2 && typeof requestId === "string") {
								setImmediate(() => pi.events.emit("subagent:result-intercom-delivery", { requestId, delivered: true }));
							}
						}
					},
			...
			const watcher = createResultWatcher(pi, state, resultsDir, 60_000, {
				timers: {
					setTimeout(handler: () => void, _ms?: number) { return setTimeout(handler, 10); },
					clearTimeout,
					setInterval,
					clearInterval,
				},
			});
			const resultPath = path.join(resultsDir, "async-2.json");
			const originalError = console.error;
			const logged: unknown[][] = [];
			console.error = (...args: unknown[]) => {
				logged.push(args);
			...
			try {
				...
				while (true) {
					const sawWarning = logged.some((entry) => /Keeping result file for retry/.test(String(entry[0] ?? "")));
					const sawCompletion = emitted.some((entry) => entry.event === "subagent:async-complete");
					if ((intercomAttempts >= 2 && sawWarning && sawCompletion) || Date.now() > deadline) break;
			...
			assert.equal(intercomAttempts, 2);
			assert.equal(emitted.filter((entry) => entry.event === "subagent:result-intercom").length, 2);
			assert.equal(emitted.some((entry) => entry.event === "subagent:async-complete"), true);
			assert.equal(logged.some((entry) => /Keeping result file for retry/.test(String(entry[0] ?? ""))), true);
			assert.equal(fs.existsSync(resultPath), false);
		} finally {
			fs.rmSync(resultsDir, { recursive: true, force: true });
		}
	...
	it("emits child terminal events from a completed parallel result even when the job is no longer tracked", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-result-watcher-child-events-"));
		try {
			...
			const state = createState();
			state.currentSessionId = "session-current";
			const resultPath = path.join(resultsDir, "parallel-failed.json");
			fs.writeFileSync(resultPath, JSON.stringify({
				id: "parallel-failed",
				runId: "parallel-failed",
				agent: "parallel:scout+scout",
				sessionId: "session-current",
				success: false,
				state: "failed",
				summary: "parallel failed",
				results: [
					{ agent: "scout", success: false, error: "first failed", output: "first output", sessionFile: "/tmp/child-0.jsonl" },
					{ agent: "scout", success: false, error: "second failed", output: "second output", sessionFile: "/tmp/child-1.jsonl" },
				],
			}), "utf-8");
			...
			const childEvents = emitted.filter((entry) => entry.event === "subagent:async-child-complete");
			assert.equal(childEvents.length, 2);
			assert.deepEqual(childEvents.map((entry) => entry.data.taskIndex), [0, 1]);
			assert.deepEqual(childEvents.map((entry) => entry.data.status), ["failed", "failed"]);
			assert.equal(emitted.filter((entry) => entry.event === "subagent:async-complete").length, 1);
			assert.equal(fs.existsSync(resultPath), false);
		} finally {
			fs.rmSync(resultsDir, { recursive: true, force: true });
