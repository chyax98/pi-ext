...
describe("chain execution — sequential", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("ignores explicit checked acceptance and preserves child output", async () => {
	...
	it("ignores explicit verified acceptance commands", async () => {
		mockPi.onCall({ output: "implemented" });
		const agents = [makeAgent("worker")];
		...
		assert.ok(!result.isError, `chain should succeed: ${JSON.stringify(result.content)}`);
		assert.equal(result.details.results[0]?.acceptance?.status, "not-required");
		assert.equal(result.details.results[0]?.finalOutput, "implemented");
	});
	...
	it("marks dynamic fanout acceptance as not required", async () => {
		mockPi.onCall({
			output: "targets",
			structuredOutput: { items: [{ path: "src/a.ts" }, { path: "src/b.ts" }] },
		});
	...
	it("treats structured_output as optional when outputSchema is set", async () => {
		const schema = {
			type: "object",
			required: ["ok"],
		...
		assert.equal(missing.details.results[0]?.finalOutput, "prose only");
		assert.equal(missing.details.results[0]?.structuredOutput, undefined);
		...
		assert.equal(invalid.details.results[0]?.finalOutput, "invalid");
		assert.equal(invalid.details.results[0]?.structuredOutput, undefined);
		assert.equal(invalid.details.workflowGraph?.nodes[0]?.status, "completed");
