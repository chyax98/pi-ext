...
describe("subagent async widget rendering", () => {
	...
	it("shows model and thinking for active async widget rows", () => {
		...
		const text = lines.join("\n");
		assert.match(text, /Agent 1\/2: reviewer · running \(slow-model · thinking high\)/);
		assert.match(text, /Agent 2\/2: scout · running \(fast-model · thinking low\)/);
		...
		const lines = buildWidgetLines([
			{
				...
				stepsTotal: 1,
				steps: [
					{ agent: "reviewer", status: "running", model: "provider/very-long-model-name-with-many-segments-20260501:high" },
				],
			},
		], theme, 68);
