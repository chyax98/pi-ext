...
import { ASYNC_DIR, INTERCOM_DETACH_REQUEST_EVENT, INTERCOM_DETACH_RESPONSE_EVENT, RESULTS_DIR } from "../../src/shared/types.ts";
import {
...
import { requestLatestForegroundDetach } from "../../src/runs/foreground/foreground-detach.ts";
...
describe("single sync execution", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("preserves implementation output even when no mutation attempt occurred", async () => {
	...
	it("preserves future-tense implementation summaries instead of judging them", async () => {
		mockPi.onCall({ output: "I’ll do that now and report back after implementing." });
		const agents = [makeAgent("worker")];

		const result = await runSync(tempDir, agents, "worker", "Implement the approved fixes", {
			runId: "no-guard-future-tense",
		});

		assert.equal(result.exitCode, 0);
		assert.equal(result.error, undefined);
		assert.equal(result.finalOutput, "I’ll do that now and report back after implementing.");
	});
	...
	it("does not require bash-enabled agents to mutate files", async () => {
		mockPi.onCall({ output: "cold start test after patch" });
		const agents = [
			makeAgent("test-runner", { tools: ["read", "grep", "bash", "ls"] }),
		];

		const result = await runSync(tempDir, agents, "test-runner", "Run cold start test after patch", {
			runId: "no-guard-bash",
		});

		assert.equal(result.exitCode, 0);
		assert.equal(result.error, undefined);
		assert.equal(result.finalOutput, "cold start test after patch");
		assert.equal(result.progress.status, "completed");
	});
	...
	it("detaches active foreground run to async status/result files", async () => {
		fs.rmSync(path.join(ASYNC_DIR, "hotkey-detach"), { recursive: true, force: true });
		fs.rmSync(path.join(RESULTS_DIR, "hotkey-detach.json"), { force: true });
		mockPi.onCall({
			steps: [
				{ delay: 100, jsonl: [events.assistantMessage("still running")] },
				{ delay: 100, jsonl: [events.assistantMessage("background done")] },
			],
		});
		const agents = makeAgentConfigs(["echo"]);
		const runPromise = runSync(tempDir, agents, "echo", "Task", { runId: "hotkey-detach" });
		...
		assert.equal(detachResult?.asyncId, "hotkey-detach");
		const result = await runPromise;
		assert.equal(result.detached, true);

		const statusPath = path.join(ASYNC_DIR, "hotkey-detach", "status.json");
		const resultPath = path.join(RESULTS_DIR, "hotkey-detach.json");
		...
		const status = JSON.parse(fs.readFileSync(statusPath, "utf8"));
		const asyncResult = JSON.parse(fs.readFileSync(resultPath, "utf8"));
		assert.equal(status.runId, "hotkey-detach");
		assert.equal(status.state, "complete");
		assert.equal(asyncResult.id, "hotkey-detach");
		assert.equal(asyncResult.success, true);
		assert.match(asyncResult.summary, /background done/);
	});
