...
interface AsyncJobTrackerModule {
	createAsyncJobTracker(
		...
		restoreActiveJobs(ctx?: unknown, restoreOptions?: { replace?: boolean }): number;
	};
}
...
describe("async job tracker", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("restores active session jobs from disk after the widget state is lost", () => {
		...
		try {
			...
			const ui = createUiContext();
			const recorder = createEventRecorder();
			const runDir = path.join(asyncRoot, "run-restore");
			...
			fs.writeFileSync(path.join(runDir, "status.json"), JSON.stringify({
				...
				steps: [
					{ agent: "scout", status: "running", currentTool: "read" },
					{ agent: "reviewer", status: "pending" },
				],
			}));
			fs.mkdirSync(path.join(asyncRoot, "wrong-session"), { recursive: true });
			fs.writeFileSync(path.join(asyncRoot, "wrong-session", "status.json"), JSON.stringify({
				runId: "wrong-session",
			...
			const restored = tracker.restoreActiveJobs(ui.ctx as never, { replace: true });

			assert.equal(restored, 1);
			assert.equal(state.asyncJobs.size, 1);
			const job = state.asyncJobs.get("run-restore");
			assert.equal(job?.status, "running");
			assert.deepEqual(job?.agents, ["scout", "reviewer"]);
			assert.equal(job?.activeParallelGroup, true);
			assert.equal(ui.widgets.length > 0, true);
		} finally {
			removeTempDir(asyncRoot);
		}
	});
	...
	it("emits one child-complete event when a parallel child reaches terminal state", async () => {
		const asyncRoot = createTempDir("pi-async-job-tracker-");
		try {
			const runDir = path.join(asyncRoot, "run-child-terminal");
			fs.mkdirSync(runDir, { recursive: true });
			fs.writeFileSync(path.join(runDir, "output-1.log"), "child done", "utf-8");
			fs.writeFileSync(path.join(runDir, "status.json"), JSON.stringify({
				...
				parallelGroups: [{ start: 0, count: 2, stepIndex: 0 }],
				steps: [
					{ agent: "a", status: "running", startedAt: 1000 },
					{ agent: "b", status: "failed", startedAt: 1000, endedAt: 2000, durationMs: 1000, exitCode: 1, error: "B failed", sessionFile: "/tmp/b-session.jsonl" },
				],
			}), "utf-8");
			...
			tracker.resetJobs(ui.ctx as never);
			tracker.handleStarted({ id: "run-child-terminal", asyncDir: runDir, mode: "parallel", agents: ["a", "b"], sessionId: "session-1" });

			await new Promise((resolve) => setTimeout(resolve, 50));
			await new Promise((resolve) => setTimeout(resolve, 30));

			const childEvents = recorder.events.filter((entry) => entry.channel === "subagent:async-child-complete");
			assert.equal(childEvents.length, 1);
			const payload = childEvents[0]!.data as { id?: string; index?: number; status?: string; error?: string; outputFile?: string; sessionFile?: string };
			assert.equal(payload.id, "run-child-terminal");
			assert.equal(payload.index, 1);
			assert.equal(payload.status, "failed");
			assert.equal(payload.error, "B failed");
			assert.equal(payload.outputFile, path.join(runDir, "output-1.log"));
			assert.equal(payload.sessionFile, "/tmp/b-session.jsonl");
		} finally {
			removeTempDir(asyncRoot);
		}
	});
	...
	it("schedules cleanup for terminal root jobs even when nested refresh fails and no live nested children are known", async () => {
		const asyncRoot = createTempDir("pi-async-job-terminal-nested-refresh-");
		let tracker: ReturnType<AsyncJobTrackerModule["createAsyncJobTracker"]> | undefined;
		const originalError = console.error;
		console.error = () => {};
		try {
			const runDir = path.join(asyncRoot, "run-terminal-nested-refresh");
			fs.mkdirSync(runDir, { recursive: true });
			fs.writeFileSync(path.join(runDir, "status.json"), JSON.stringify({
				runId: "run-terminal-nested-refresh",
				mode: "single",
				state: "complete",
				startedAt: Date.now() - 1000,
				lastUpdate: Date.now(),
			...
			tracker.handleStarted({
				id: "run-terminal-nested-refresh",
				asyncDir: runDir,
				agent: "worker",
				nestedRoute: {
					rootRunId: "run-terminal-nested-refresh",
					eventSink: path.join(asyncRoot, "not-contained-events"),
					controlInbox: path.join(asyncRoot, "not-contained-controls"),
					capabilityToken: "bad-token",
				},
			...
			assert.equal(state.asyncJobs.get("run-terminal-nested-refresh")?.status, "complete");
			assert.equal(state.cleanupTimers.has("run-terminal-nested-refresh"), true);
		} finally {
			console.error = originalError;
			tracker?.resetJobs();
			removeTempDir(asyncRoot);
