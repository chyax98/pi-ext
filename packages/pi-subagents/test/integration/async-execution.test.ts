...
interface AsyncResultPayload {
	...
	aggregate?: { status?: string; usable?: number; failed?: number; warnings?: number; total?: number; message?: string; failedIndexes?: number[]; warningIndexes?: number[] };
}
...
describe("async execution utilities", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("background read-only parallel fanout is partial usable when one scout fails", { skip: !isAsyncAvailable() || !createSubagentExecutor ? "jiti or executor not available" : undefined }, async () => {
		mockPi.onCall({ output: "scout one findings" });
		mockPi.onCall({ output: "provider timeout", exitCode: 1 });
		mockPi.onCall({ output: "scout three findings" });
		const executor = createSubagentExecutor!({
			pi: { events: createEventBus(), getSessionName: () => undefined },
			state: { baseCwd: tempDir, currentSessionId: null, asyncJobs: new Map(), foregroundControls: new Map(), lastForegroundControlId: null },
			config: {},
			...
			getSubagentSessionRoot: () => tempDir,
			expandTilde: (p: string) => p,
			discoverAgents: () => ({ agents: [makeAgent("scout")] }),
		});

		const result = await executor.execute(
			"async-partial-scouts",
			{
				tasks: [
					{ agent: "scout", task: "Inspect alpha" },
					{ agent: "scout", task: "Inspect beta" },
					{ agent: "scout", task: "Inspect gamma" },
				],
				async: true,
				clarify: false,
			},
		...
		assert.equal(payload.aggregate?.status, "partial");
		assert.equal(payload.aggregate?.failedIndexes?.length, 1);
		assert.equal(payload.aggregate?.usable, 2);
		assert.equal(payload.aggregate?.failed, 1);
		assert.match(payload.summary ?? "", /Partial success/);
		assert.equal(payload.results.filter((entry) => entry.success === false).length, 1);
		assert.equal(status.state, "complete");
		assert.equal(status.aggregate?.status, "partial");
		assert.equal(status.steps?.filter((entry) => entry.status === "failed").length, 1);
	});

	it("async single ignores explicit reviewed acceptance and returns child output", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		...
		assert.equal(result.results[0]?.output, "implemented");
		assert.equal(result.results[0]?.acceptance?.status, "not-required");
		assert.equal(status.steps?.[0]?.acceptance?.status, "not-required");
	});
	...
	it("background implementation runs preserve output when no mutation attempt occurred", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		...
		assert.equal(payload.exitCode, 0);
		assert.equal(payload.results[0].success, true);
		assert.equal(payload.results[0].output, "I’ll do that now and report back after implementing.");
	});

	it("background bash-enabled agents do not need a mutation opt-out", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		mockPi.onCall({ output: "cold start test after patch" });

		const id = `async-no-guard-bash-${Date.now().toString(36)}`;
		const sessionRoot = path.join(tempDir, "sessions");
