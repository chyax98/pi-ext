...
describe("buildPiArgs system prompt mode wiring", () => {
  ...
  it("supports isolated child extension policy with include and exclude", () => {
    const { args } = buildPiArgs({
      baseArgs: ["-p"],
      task: "hello",
      sessionEnabled: false,
      inheritProjectContext: false,
      inheritSkills: false,
      tools: ["read", "./tool-ext.ts"],
      extensions: ["./agent-ext.ts", "./blocked-ext.ts"],
      extensionPolicy: {
        mode: "isolated",
        include: ["./included-ext.ts"],
        exclude: ["blocked-ext.ts", "tool-ext.ts"],
      },
    });
    ...
    assert.ok(extensionArgs.some((arg) => arg.endsWith(path.join("src", "runs", "shared", "subagent-prompt-runtime.ts"))));
    assert.ok(extensionArgs.includes("./agent-ext.ts"));
    assert.ok(extensionArgs.includes("./included-ext.ts"));
    assert.equal(extensionArgs.includes("./blocked-ext.ts"), false);
    assert.equal(extensionArgs.includes("./tool-ext.ts"), false);
  });

  it("supports inherited child extension policy for explicit opt-in", () => {
    const { args } = buildPiArgs({
      baseArgs: ["-p"],
      task: "hello",
      sessionEnabled: false,
      inheritProjectContext: false,
      inheritSkills: false,
      extensionPolicy: { mode: "inherit", include: ["./extra-ext.ts"] },
    });

    const extensionArgs = args.filter((arg, index) => args[index - 1] === "--extension");
    assert.equal(args.includes("--no-extensions"), false);
    assert.ok(extensionArgs.some((arg) => arg.endsWith(path.join("src", "runs", "shared", "subagent-prompt-runtime.ts"))));
    assert.ok(extensionArgs.includes("./extra-ext.ts"));
  });
