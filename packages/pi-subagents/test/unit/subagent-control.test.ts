...
describe("subagent control attention state", () => {
	...
	it("supports a specific attention message", () => {
		const event = buildControlEvent({
			...
			runId: "run-1",
			agent: "worker",
			message: "worker needs parent input",
		});

		assert.equal(event.message, "worker needs parent input");
	});
	...
	it("formats control notices with a proactive hint and concrete commands", () => {
		...
		assert.match(message, /Status: agent_status\(\{ id: "78f659a3" \}\)/);
		assert.match(message, /Stop: agent_stop\(\{ id: "78f659a3", reason: "needs attention", mode: "soft" \}\)/);
