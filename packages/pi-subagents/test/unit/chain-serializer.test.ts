...
`, "project", "/tmp/review-chain.md"),
	...
	it("preserves legacy acceptance fields without validating them", () => {
		const parsed = parseJsonChain(JSON.stringify({
			name: "legacy-acceptance-chain",
			description: "Chain with ignored legacy acceptance fields",
			chain: [
				{ agent: "worker", task: "Fix bug", acceptance: { level: "checked", evidence: ["changed-files", "commands-run"] } },
				{
					parallel: [
						{ agent: "reviewer", task: "Review", acceptance: { level: "none" } },
					],
				},
			],
		}), "project", "/tmp/legacy-acceptance-chain.chain.json");

		assert.deepEqual((parsed.steps[0] as { acceptance?: unknown }).acceptance, { level: "checked", evidence: ["changed-files", "commands-run"] });
		assert.deepEqual(((parsed.steps[1] as { parallel?: Array<{ acceptance?: unknown }> }).parallel?.[0]?.acceptance), { level: "none" });
	});
});
