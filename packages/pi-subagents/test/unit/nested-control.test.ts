...
import { createSubagentExecutor, resolveNestedResumeCwd } from "../../src/runs/foreground/subagent-executor.ts";
...
describe("nested control routing", () => {
	...
	it("renders nested children in foreground status output", async () => {
		...
		try {
			...
			assert.match(text(result), /Status: agent_status\(\{ id: "nested-foreground" \}\)/);
		} finally {
			fs.rmSync(root, { recursive: true, force: true });
		}
	});
	...
	it("resolves terminal nested resume cwd from nested async status", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-nested-cwd-"));
		try {
			const runId = "nested-cwd";
			const asyncDir = path.join(TEMP_ROOT_DIR, "nested-subagent-runs", "root-control", runId);
			const expectedCwd = path.join(root, "workdir");
			fs.mkdirSync(asyncDir, { recursive: true });
			fs.mkdirSync(expectedCwd, { recursive: true });
			fs.writeFileSync(path.join(asyncDir, "status.json"), JSON.stringify({ runId, state: "complete", cwd: expectedCwd }));

			const cwd = resolveNestedResumeCwd("root-control", {
				id: runId,
				state: "complete",
				agent: "worker",
				asyncDir,
			...
			fs.rmSync(root, { recursive: true, force: true });
			fs.rmSync(path.join(TEMP_ROOT_DIR, "nested-subagent-runs", "root-control", "nested-cwd"), { recursive: true, force: true });
		}
	});
