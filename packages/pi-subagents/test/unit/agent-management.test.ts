...
describe("agent management config parsing", () => {
	...
	it("creates, gets, updates, and deletes a packaged agent by runtime name", () => {
		...
		const filePath = path.join(tempDir, ".pi", "agent", "agents", "code-analysis.scout.md");
		...
		const updatedPath = path.join(tempDir, ".pi", "agent", "agents", "documentation.scout.md");
	...
	it("creates and updates packaged chains while preserving packaged step names", () => {
		const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
		fs.mkdirSync(path.join(tempDir, ".pi", "agent", "agents"), { recursive: true });
		fs.writeFileSync(path.join(tempDir, ".pi", "agent", "agents", "code-analysis.scout.md"), `---
name: scout
...
`, "utf-8");
	...
	it("creates delegate with its builtin prompt defaults", () => {
		...
		const filePath = path.join(tempDir, ".pi", "agent", "agents", "delegate.md");
