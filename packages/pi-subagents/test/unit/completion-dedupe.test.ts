...
describe("buildCompletionKey", () => {
	...
	it("scopes id keys by session when available", () => {
		const a = buildCompletionKey({ id: "run-123", sessionId: "session-a" }, "fallback");
		const b = buildCompletionKey({ id: "run-123", sessionId: "session-b" }, "fallback");
		assert.equal(a, "session:session-a:id:run-123");
		assert.equal(b, "session:session-b:id:run-123");
		assert.notEqual(a, b);
	});
