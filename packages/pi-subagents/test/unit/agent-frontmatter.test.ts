...
describe("agent frontmatter defaultContext", () => {
	...
	it("parses defaultContext from discovered agent frontmatter", () => {
		...
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agent", "agents");
...
`, "utf-8");
	...
	it("bundles only the small managed-agent role set", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-subagents-builtin-role-set-"));
		tempDirs.push(dir);
		const names = discoverAgentsAll(dir).builtin.map((agent) => agent.name).sort();
		assert.deepEqual(names, ["delegate", "planner", "reviewer", "scout", "worker"]);
	});
});
...
describe("packaged agent and chain discovery", () => {
	it("recursively discovers nested project agents while keeping chain files separate", () => {
		...
		tempDirs.push(dir);
		const nestedDir = path.join(dir, ".pi", "agent", "agents", "code-analysis", "deep");
...
`;
	...
	it("keeps packaged and un-packaged runtime names distinct while preserving un-packaged precedence", () => {
		...
		fs.mkdirSync(path.join(dir, ".pi", "agent", "agents"), { recursive: true });
...
Legacy
`, "utf-8");
		fs.writeFileSync(path.join(dir, ".pi", "agent", "agents", "scout.md"), `---
name: scout
...
Project
`, "utf-8");
		fs.writeFileSync(path.join(dir, ".pi", "agent", "agents", "packaged.md"), `---
name: scout
package: code-analysis
...
`, "utf-8");
		...
		assert.equal(unqualified?.filePath, path.join(dir, ".pi", "agent", "agents", "scout.md"));
...
describe("project agent directory discovery", () => {
	it("discovers project agents only from .pi/agent/agents", () => {
...
Legacy prompt
`, "utf-8");
		fs.writeFileSync(path.join(dir, ".pi", "agent", "agents", "canonical.md"), `---
name: canonical
...
`, "utf-8");
		...
		const result = discoverAgents(dir, "project");
		assert.equal(result.agents.some((agent) => agent.name === "legacy"), false);
		assert.ok(result.agents.find((agent) => agent.name === "canonical" && agent.filePath === path.join(dir, ".pi", "agent", "agents", "canonical.md")));
		assert.equal(result.projectAgentsDir, path.join(dir, ".pi", "agent", "agents"));
	});

	it("uses project .pi/agent/agents instead of .agents on name collisions", () => {
...
Legacy prompt
`, "utf-8");
		fs.writeFileSync(path.join(dir, ".pi", "agent", "agents", "shared.md"), `---
name: shared
...
`, "utf-8");
		...
		assert.ok(shared);
		assert.equal(shared.filePath, path.join(dir, ".pi", "agent", "agents", "shared.md"));
		assert.equal(shared.description, "Canonical shared");
		assert.equal(shared.systemPrompt.trim(), "Canonical prompt");
	});

	it("uses the project root for the canonical project agent dir when .pi exists", () => {
		...
		assert.equal(result.projectDir, path.join(dir, ".pi", "agent", "agents"));
	});
	...
	it("discovers project chains from .pi/chains", () => {
		...
		fs.writeFileSync(path.join(dir, ".pi", "agent", "agents", "ignored.chain.md"), `---
name: ignored-chain
