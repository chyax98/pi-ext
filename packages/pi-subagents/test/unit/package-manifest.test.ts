...
test("package exposes prompt injection resources but no pi-subagents skill", () => {
	const packageJson = JSON.parse(fs.readFileSync(path.join(projectRoot, "package.json"), "utf-8"));

	assert.equal(packageJson.pi?.skills, undefined);
	assert.ok(packageJson.pi?.prompts?.includes("./prompts"));
	assert.ok(packageJson.pi?.extensions?.includes("./src/extension/index.ts"));
	assert.equal((packageJson.files ?? []).some((entry: string) => entry.startsWith("skills")), false);
