...
function failedResult(agent: string, output: string) {
	return { ...result(agent, output), exitCode: 1, error: output };
}
...
test("parallel best-effort partial aggregate does not render root failure", () => {
	const text = componentText(renderSubagentResult({
		content: [{ type: "text", text: "Partial success: 2/3 usable" }],
		details: {
			mode: "parallel",
			results: [result("scout", "a"), failedResult("scout", "provider timeout"), result("scout", "c")],
	...
	assert.doesNotMatch(text, /^✗ parallel/m);
	assert.match(text, /Usable results preserved/);
	assert.match(text, /Agent 2\/3: scout/);
