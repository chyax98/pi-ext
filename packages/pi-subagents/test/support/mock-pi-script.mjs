...
function isJsonMode(args) {
	for (let i = 0; i < args.length; i++) {
		if (args[i] === "--mode") return args[i + 1] === "json";
	}
	return false;
}
...
async function main() {
	...
	if (typeof response.delay === "number" && response.delay > 0) await new Promise((resolve) => setTimeout(resolve, response.delay));

	if (Array.isArray(response.steps) && response.steps.length > 0) {
		for (const step of response.steps) {
			if (typeof step?.delay === "number" && step.delay > 0) await new Promise((resolve) => setTimeout(resolve, step.delay));
			if (Array.isArray(step?.jsonl) && step.jsonl.length > 0) await writeResponseEntries(step.jsonl, jsonMode);
		...
		if (jsonMode) await writeJsonlLine(defaultAssistantMessage(response.output));
		else await writeStdout(`${response.output}\n`);
	}
	await maybeWriteStructuredOutput(response, jsonMode);

	if (typeof response.stderr === "string" && response.stderr.length > 0) process.stderr.write(response.stderr);
