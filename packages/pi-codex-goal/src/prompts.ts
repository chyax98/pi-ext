...
export const TOOL_PROMPT_GUIDELINES = [
  ...
  "Token budget rule for create_goal: only set token_budget when the user explicitly gives a budget/limit in natural language; otherwise omit it entirely. Budgets are in millions of tokens by default: phrases like 20m, 20 m, 预算 20, or 激进 20m 完成这个任务 mean token_budget 20, which the tool stores as 20,000,000 tokens. Do not choose a small conservative budget on your own.",
  "Execution posture for broad goals: default to aggressive vertical-slice progress unless the user naturally asks for normal/careful/small-step execution. Do not make tiny isolated edits for hours while the main workstreams remain untouched; use delegation for independent workstreams when available and keep going until final gates pass or a real blocker is proven.",
...
export function compactContinuationPrompt(goal: ThreadGoal): string {
  return [
    ...
    "",
    "Avoid repeating work that is already done. Choose the next concrete action toward the objective. For broad refactors, migrations, audits, or multi-subsystem goals, choose a meaningful vertical slice or final-gate blocker, not a tiny cosmetic cleanup.",
    "",
