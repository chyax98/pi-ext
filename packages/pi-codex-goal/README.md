...
For local development inside this piext monorepo, install the workspace package directory in Pi:
...
Install only one copy at a time. Duplicate local/npm/GitHub installs all try to register `get_goal`, `create_goal`, and `update_goal`, which causes tool-registration conflicts.

Compatibility note: this workspace is currently tested against Pi 0.79.4 on Node 24. Pi-bundled runtime packages remain optional wildcard peers, so npm peer ranges do not hard-block newer Pi releases; runtime behavior is only verified against the tested baseline until a follow-up package update confirms it.
